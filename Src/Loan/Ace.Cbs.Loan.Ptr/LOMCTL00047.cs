...
{
    public class LOMCTL00047 : AbstractPresenter, ILOMCTL00047
    {
        #region Properties
        private ILOMVEW00047 view;
        public ILOMVEW00047 View
        {
            get { return this.view; }
            set { this.WireTo(value); }
        }

        private void WireTo(ILOMVEW00047 view)
        {
            if (this.view == null)
            {
                this.view = view;
        ...
        public LOMDTO00047 GetEntity()
        {
            LOMDTO00047 entity = new LOMDTO00047();
            ...
            if (this.Validate_Form())
            {
                if (!CXCOM00006.Instance.IsExceedTodayDate(this.view.StartDate))
                {
                    if (!CXCOM00006.Instance.IsExceedTodayDate(this.view.EndDate))
                    {
                        ...
                        {
                            LOMDTO00047 DTO = this.GetEntity();
                            IList<LOMDTO00047> DTOList = CXClientWrapper.Instance.Invoke<ILOMSVE00047, IList<LOMDTO00047>>(x => x.SelectLoansRepaymentScheduleByDate(DTO));
                            if (DTOList.Count > 0)
                            {
                                ...
                                string header = "Loans Repayment Schedule By Date";
                                CXUIScreenTransit.Transit("frmLOMVEW00048.ReportViewer", true, new object[] { DTOList, startDate, endDate, currency, sourcebranch, header });
                            }
                            else
                            {
