...
{
    public class LOMCTL00059 : AbstractPresenter, ILOMCTL00059
    {
        #region Properties
        private ILOMVEW00059 view;
        public ILOMVEW00059 View
        {
            get { return this.view; }
            set { this.WireTo(value); }
        }

        private void WireTo(ILOMVEW00059 view)
        {
            if (this.view == null)
            {
                this.view = view;
                ...
                if (!CXCOM00006.Instance.IsExceedTodayDate(this.view.StartDate))
                {
                    if (!CXCOM00006.Instance.IsExceedTodayDate(this.view.EndDate))
                    {
                        ...
                        else
                        {
                            ...
                            IList<LOMDTO00059> DTOList = CXClientWrapper.Instance.Invoke<ILOMSVE00059, IList<LOMDTO00059>>(x => x.SelectODLimitChangeByDate(DTO));
                            if (DTOList.Count > 0)
                            {
                                ...
                                string header = "Overdraft Limit Change Listing";
                                CXUIScreenTransit.Transit("frmLOMVEW00060.ReportViewer", true, new object[] { DTOList, startDate, endDate, currency, sourcebranch, header });
