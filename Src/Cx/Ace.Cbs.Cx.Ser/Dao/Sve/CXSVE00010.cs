...
namespace Ace.Cbs.Cx.Ser.Sve
{
    ...
    public class CXSVE00010 : BaseService, ICXSVE00010
    {
        ...
        [Transaction(TransactionPropagation.Required)]
        public bool AllUpdateForWithdraw(string eno, string acctno, decimal amount, decimal Oamount, bool wStatus, string chequeNo, string userId, string soruceBr, string channel, DateTime settlementDate,bool isAutoLink, string errorMessage, int createdUserId,bool isLastWithdraw)
        {

            string code = this.dataGenerateDAO.AllUpdateForWithdrawal(eno, acctno, amount, Oamount, chequeNo, soruceBr, settlementDate, channel,isAutoLink, createdUserId,isLastWithdraw);
        ...
        public string UpdateForTransfer(string voucherNo, string accountNo, decimal amount, decimal oAmount, string chequeNo, string voucherStatus, bool status, string narration, string userId, string sourceCur, decimal homeExchangeRate, string sourceBr, DateTime settlementDate, string channel)
        {
            ...
            if (code != "TRANSFER VOUCHER PROCESS COMPLETE!...")
            {
                code = this.TranslateToCXMessageCode(code);
            }
        ...
        {
            string code = this.dataGenerateDAO.UpdateForHPIntPrepaymentVoucher(voucherNo, accountNo, amount, narration, userId, sourceCur, sourceBr, settlementDate, channel);
        ...
        {
            string code = this.DataGenerateDAO.TransferDebitVoucher(voucherNo, accountNo, isVIP, amount, chequeNo, voucherStatus, transactionCode, reference, debitBranch, creditBranch, creditAccount, comissionCharges, communicationCharges, status, iBSAC, incomeAC, faxAC,takeIncome,narration, userId, sourceCur, homeExchangeRate, sourceBr, settlementDate, channel);
            code = this.TranslateToCXMessageCode(code);
        ...
        {
            this.DataGenerateDAO.NPLServiceCharges(servicechargedto);
            return true;
        ...
        //Change TLF,MTLF,YTLF,HTLF when shut down
        [Transaction(TransactionPropagation.Required)]
        public bool Sp_ChangeTables(DateTime lastSettlementDate, DateTime nextSettlementDate,string sourcebr)
        {
            this.DataGenerateDAO.SP_ChangeTables(lastSettlementDate, nextSettlementDate, sourcebr);
            return true;
        }

        //Change CLedger,MonthlyCledger,YearlyCledger,BudgelyCledger when shut down //Updated By HWKO (17-Mar-2017)
        [Transaction(TransactionPropagation.Required)]
        public bool Sp_ChangeTables_AllCLedger(DateTime lastSettlementDate, DateTime nextSettlementDate, string sourcebr,string budget)
        {
            this.DataGenerateDAO.Sp_ChangeTables_AllCLedger(lastSettlementDate, nextSettlementDate, sourcebr,budget);
            return true;
        }
        ...
        [Transaction(TransactionPropagation.Required)]
        public IList<TLMDTO00015> CashDenominationListing_ForMultiTransaction(string currency, string whereString, string orderString)
        {

            try
            {
                return this.dataGenerateDAO.CashDenominationListing_ForMultiTrasaction(currency, whereString, orderString);
            }
            catch (Exception ex)
            {
                this.ServiceResult.ErrorOccurred = true;
        ...
        {
           string message = this.DataGenerateDAO.Sp_ALUpdate_Int_TransferAdjust(neweno,eno,accountno,amount,cheque,userno,tstatus,voustatus,channel,refvno,reftype,sourcebr,rate,cur,settlementdate,createdDate);
           return message;
        }

        [Transaction(TransactionPropagation.Required)]   //ASDA (Sp_SERVICECHARGES_VOU)
        public bool Sp_SERVICECHARGES_VOU(string eNO,string mAcctNo,string lno,string narration,decimal amount,decimal oAmount,string userNo,string vouStatus,
        ...
        {
            string code = this.DataGenerateDAO.Sp_SERVICECHARGES_VOU(eNO, mAcctNo, lno, narration, amount, oAmount, userNo, vouStatus,
             tStatus, cur, homeExRate, sourceBr, settlementDate, channel, returnValue, message);            
            if (code.ToString() == "False")
            {
                this.ServiceResult.ErrorOccurred = false;  // Transaction is not Success!
                return true;
            }  
            else 
            {
                this.ServiceResult.ErrorOccurred = true;  // Transaction is not Success!               
                return false;
            }
        }
