...
{
    public class TLMCTL00010 : AbstractPresenter, ITLMCTL00010
    {
        #region Prorerties

        private ITLMVEW00010 view;
        public IList<PFMDTO00001> AccountInformationList { get; set; }
        int printedLine = 0 ;

        public ITLMVEW00010 View
        {
            get { return this.view; }
            set { this.WireTo(value); }
        }
        private void WireTo(ITLMVEW00010 view)
        {
            if (this.view == null)
            {
                this.view = view;
        ...
        public void mtxtAccountNo_CustomValidating(object sender, ValidationEventArgs e)
        {
            try
            {
                ...
                {
                            AccountInformationList = CXClientWrapper.Instance.Invoke<ITLMSVE00010, IList<PFMDTO00001>>(x => x.SelectByAccountNumber(this.View.AccountNo));
        ...
        //    {
        //        // Do Data to deposit.
        //        this.SetCustomErrorMessage(this.GetControl("gvDepositInformation"), CXMessage.MV00107);
        //    }
        //}
        ...
        public bool Save()
        {
            ...
            else
            {
                if (CXUIScreenTransit.Transit("frmTLMVEW00011", true, new object[] { this.View.TotalAmount, this.View.Currency, CXDMD00008.Received, "frmTLMVEW00011" }) == DialogResult.OK)
                {
                    ...
                    if (this.View.LocationTransactions)
                        Tlf_List = CXClientWrapper.Instance.Invoke<ITLMSVE00010, IList<PFMDTO00054>>(x => x.SaveDepositLocal(this.View.DenoCollection, this.View.DepositEntity));
                    else
                        Tlf_List = CXClientWrapper.Instance.Invoke<ITLMSVE00010, IList<PFMDTO00054>>(x => x.SaveDepositOnline(this.View.DenoCollection, this.View.DepositEntity, CurrentUserEntity.BranchCode));
                    ...
                    if (CXClientWrapper.Instance.ServiceResult.ErrorOccurred)
                    {
                        ...
                        if (this.View.LocationTransactions)
                        {
                            ...
                            for (int i = 0; i < this.View.DenoCollection.Count; i++)
                            {
                                ...
                                logItemForTlf[10] = this.View.DenoCollection[i].BranchCode;//SourceBr
            ...
            try
            {
                ...
                foreach (TLMDTO00038 info in this.View.DenoCollection)
                {
                    ...
                    if (query.Count<PFMDTO00043>() > 0)
                    {
                        ...
                        if (!CXClientWrapper.Instance.Invoke<ITLMSVE00010, bool>(x => x.UpdateCleadgerPrintLineNoandDeletePrnFile(info.AccountNo,CurrentUserEntity.CurrentUserID,lineNo)))
                        { Ace.Windows.CXClient.CXUIMessageUtilities.ShowMessageByCode(CXClientWrapper.Instance.ServiceResult.MessageCode); }
                    }
                }
            }
        ...
        private TLMDTO00038 GetDepositInfoToSave(TLMDTO00038 depositEntity)
        {
            ...
            depositEntity.FromBranchCode = this.View.LocalBranchCode;
        ...
        {
            CXUIScreenTransit.Transit("frmTLMVEW00008", true, new object[] { "frmTLMVEW00010",this.View.Amount });
        }

        public void Call_AccountInformationEnquiry()
        {
            CXUIScreenTransit.Transit("frmTLMVEW00042", true, new object[] { "frmTLMVEW00010", this.View.AccountNo });
        }
