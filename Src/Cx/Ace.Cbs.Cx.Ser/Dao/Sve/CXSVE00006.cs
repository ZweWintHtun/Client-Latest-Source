...
namespace Ace.Cbs.Cx.Ser.Sve
{
    ...
    public class CXSVE00006 : BaseService, ICXSVE00006
    {
        ...
        public bool HasDuplicatedChequeBookIssueNo(string chequeBookNo, string fromNumber, string toNumber, string branchCode,string currencyCode,string accountNo)
        {
            int count = this.codeCheckerDAO.ChequeSelectCountByBookNoAndSourceBr(chequeBookNo, branchCode,currencyCode);
            if (count > 0)
            {
            ...
            count = this.codeCheckerDAO.ChequeSelectCount(accountNo, chequeBookNo, fromNumber, toNumber, branchCode, currencyCode);
            ...
            count = this.codeCheckerDAO.UCheckSelectCount(fromNumber, toNumber, branchCode,accountNo);
            ...
            count = this.codeCheckerDAO.SCheckSelectCount(chequeBookNo, fromNumber, toNumber, branchCode,accountNo);
        ...
        public bool IsValidChequeBookIssueNoForStopCheque(string accountNumber, string chequeBookNo, string fromNumber, string toNumber, string branchCode,string cur)
        {
            int count = this.codeCheckerDAO.ChequeSelectCount(accountNumber, chequeBookNo, fromNumber, toNumber, branchCode,cur);
            ...
            count = this.codeCheckerDAO.UCheckSelectCount(fromNumber, toNumber, branchCode,accountNumber);
        ...
        public bool IsAlreadyPrintedChequeNo(string acctno,string fromNumber, string toNumber, string branchCode,string chequeBookNo)
        {
            int count = this.codeCheckerDAO.PrintChequeSelectCount(acctno, fromNumber, toNumber, branchCode, chequeBookNo);
        ...
        public bool IsValidChequeBookIssueNoForAccountClose(string accountNo, string chequeNo, string branchCode)
        {
            ...
            count = this.codeCheckerDAO.UCheckSelectCount(chequeNo, chequeNo, branchCode,accountNo);
            ...
            count = this.codeCheckerDAO.SCheckSelectCount(chequeNo, branchCode,accountNo);
        ...
        {
            return this.codeCheckerDAO.FAOFSelectByAccountNumberForRePrintingPassBook(accountNumber);
        }
        ...
        {
            return this.codeCheckerDAO.GetCurrentAccountInfoByAccountNumber_ForClosedACAndUnClosed(accountNo);
        }
        ...
        public TLMDTO00016 GetPOByPoNoandBudgetYear(string poNo, string budgetYear,string sourcebr)
        {
            ...
            else
            {
                ...
                if (poDTO == null)
                {
                    TLMDTO00001 reDTO = this.codeCheckerDAO.SelectREByPoNo(poNo, budgetYear,sourcebr);
                    if (reDTO == null)
                    {
                        this.ServiceResult.ErrorOccurred = true;
                ...
                else
                {
                    ...
                    else if (poDTO.Budget != budgetYear)
                    {
                        this.ServiceResult.ErrorOccurred = true;
        ...
        public IList<PFMDTO00001> GetCustomerInfomationByAccountNo(string accountNo,bool isPhoto)
        {
            try
            {
                ...
                else if (CXCOM00007.Instance.GetValueByKeyName(CXCOM00009.FixedAccountSign) == customerLedgerInfo.AccountSign.Substring(0, 1))
                {
                    customerList = this.CodeCheckerDAO.SelectCustomerInformationByFAOF(accountNo);
                }
                else if (CXCOM00007.Instance.GetValueByKeyName(CXCOM00009.BusinessLoanAccountSign) == customerLedgerInfo.AccountSign.Substring(0, 1))
                {
                    customerList = this.CodeCheckerDAO.GetCustomerInformationBySAOForCAOF(accountNo, customerLedgerInfo.AccountSign.Substring(0, 1));
                }
                else if (CXCOM00007.Instance.GetValueByKeyName(CXCOM00009.HirePurchaseLoanAccountSign) == customerLedgerInfo.AccountSign.Substring(0, 1))
                {
                    customerList = this.CodeCheckerDAO.GetCustomerInformationBySAOForCAOF(accountNo, customerLedgerInfo.AccountSign.Substring(0, 1));
                }
                else if (CXCOM00007.Instance.GetValueByKeyName(CXCOM00009.PersonalLoanAccountSign) == customerLedgerInfo.AccountSign.Substring(0, 1))
                {
                    customerList = this.CodeCheckerDAO.GetCustomerInformationBySAOForCAOF(accountNo, customerLedgerInfo.AccountSign.Substring(0, 1));
                }
                else if (CXCOM00007.Instance.GetValueByKeyName(CXCOM00009.DealerAccountSign) == customerLedgerInfo.AccountSign.Substring(0, 1))
                {
                    customerList = this.CodeCheckerDAO.GetCustomerInformationBySAOForCAOF(accountNo, customerLedgerInfo.AccountSign.Substring(0, 1));
                }
        ...
        public bool HasLegalCaseAccount(string accountNo)
        {
            ...
            {
                this.ServiceResult.ErrorOccurred = true;
                this.ServiceResult.MessageCode = CXMessage.MV00137; //This Account is in Legal Case.
            }
        ...
        public bool IsExpiredLoansAccount(string accountNo,DateTime todaydate)
        {
            int count = this.codeCheckerDAO.GetExpiredLoansFromLoansByAccountNo(accountNo, todaydate);
            if (count > 0)
            {
                this.ServiceResult.ErrorOccurred = true;
                this.ServiceResult.MessageCode = CXMessage.MV90059; //This loan no. is expired!
            }
        ...
        public bool CheckAmount(string accountNo,decimal amount,bool isMinBalCheck, bool isVIP,bool isDebit,ref bool isLink, ref string messageCode)
        {
            ...
            if (cledgerDTO.AccountSign.Substring(0, 1) == CXCOM00007.Instance.GetValueByKeyName(CXCOM00009.BusinessLoanAccountSign))
            {
            ...
            //Added by HWKO (17-Aug-2017)
            else if (cledgerDTO.AccountSign.Substring(0, 1) == CXCOM00007.Instance.GetValueByKeyName(CXCOM00009.HirePurchaseLoanAccountSign))
            {
                globalAmount = setupDTO.CurrentMinimumBalance;
                newSetupDTOforAmount = this.CodeCheckerDAO.SelectByVariable("CATRANAMT");
                newSetupDTOforDivider = this.CodeCheckerDAO.SelectByVariable("CADIVIDER");
            ...
            //Added by HWKO (17-Aug-2017)
            else if (cledgerDTO.AccountSign.Substring(0, 1) == CXCOM00007.Instance.GetValueByKeyName(CXCOM00009.PersonalLoanAccountSign))
            {
                globalAmount = setupDTO.CurrentMinimumBalance;
                newSetupDTOforAmount = this.CodeCheckerDAO.SelectByVariable("CATRANAMT");
                newSetupDTOforDivider = this.CodeCheckerDAO.SelectByVariable("CADIVIDER");
            ...
            //Added by HWKO (17-Aug-2017)
            else if (cledgerDTO.AccountSign.Substring(0, 1) == CXCOM00007.Instance.GetValueByKeyName(CXCOM00009.DealerAccountSign))
            {
                globalAmount = setupDTO.CurrentMinimumBalance;
                newSetupDTOforAmount = this.CodeCheckerDAO.SelectByVariable("CATRANAMT");
                newSetupDTOforDivider = this.CodeCheckerDAO.SelectByVariable("CADIVIDER");
            }
            // Comment by HWKO (17-Aug-2017)
            //if (cledgerDTO.AccountSign.Substring(0, 1) == CXCOM00007.Instance.GetValueByKeyName(CXCOM00009.BusinessLoanAccountSign))
            //{
            //    globalAmount = setupDTO.CurrentMinimumBalance;
            //    newSetupDTOforAmount = this.CodeCheckerDAO.SelectByVariable("CATRANAMT");
        ...
        /// Created By KKT
        /// UPdated By NLH
        /// Updated By ZMS (16.11.18) for Pristine requirement
        [Transaction(TransactionPropagation.Required)]
        ...
        {
            string splitStatus = string.Empty; //added by ASDA(to check online withdraw reversal case)
            #region validation
            ...
            {
                if (ActiveBranch.Contains("WithdrawReversalOnline"))
                {
                    string[] str =  ActiveBranch.Split('.');
                    splitStatus = str[1];
                    ActiveBranch = str[0];
                }
            }
            ...
            //Commented by HMW (For Seperating EOD Process)
            ...
            foreach (PFMDTO00054 tlfdto in ListTLFdto)
            {
                ...
                if (tlfdto.TransactionCode.Substring(0, 3) == "DRE")  //Added by ASDA
                {
                    ReversalTlfORM.Narration = reversaltrancodedto.Description +" ("+ tlfdto.DRegisterNo+")";
                }
                else if(tlfdto.TransactionCode.Substring(0, 3) == "ERE")
                {
                    ReversalTlfORM.Narration = reversaltrancodedto.Description + " (" + tlfdto.ERegisterNo + ")";
                }
                else 
                {
                ...
                if (!string.IsNullOrEmpty(tlfdto.AccountSign))
                {
                    ...
                    else if (tlfdto.AccountSign[0] == 'S')
                    {
                        ...
                        if (reversaltrancodedto.Status[1] == 'C')
                        {
                            ...
                            PRNFileORM.Balance = cledgerDTO.CurrentBal + cledgerDTO.OverdraftLimit + tlfdto.Amount;
                        }
                        else
                        {
                        ...
                        PRNFileORM.CreatedUserId = UpdatedUserID;
                        PRNFileORM.UserNo = UpdatedUserID.ToString();
                        //PRNFileORM.UserNo
            ...
            if (ActiveBranch == BranchCode)
            {
                ...
                TLMDTO00015 Orgncashdenodto = new TLMDTO00015();
                ...
                else
                {
                    ...
                    //TLMDTO00015 Orgncashdenodto = new TLMDTO00015();
                    if (casndenodto == null || casndenodto.WithdrawEntryStatus == "WithdrawReversal")  //added by ASDA
                    {
                        //**if transaction is online multiple with inocme by cash, there is 2 group no in cashdeno. 
                        //**but reversal only allow withdraw amount (status=P), not allow to reverse(Status=R). 
                        Orgncashdenodto = this.CodeCheckerDAO.SelectCashDenoByGroupNoAndStatus(GroupNo, BranchCode,"P");
                    }
                    else
                    {
                        Orgncashdenodto = this.CodeCheckerDAO.SelectCashDenoByGroupNo(GroupNo, BranchCode);
                    }
                        ...
                        //{
                            ...
                            if (this.CodeCheckerDAO.SelectDepoDenoChargesAmountByEntryNo(GroupNo).Count > 0)
                            {
                                if (casndenodto != null)
                                {
                                    ...
                                    //CashDenoORM.Id = cashDAO.SelectMaxId() + 1;
            ...
            if (casndenodto == null)
            {
                ...
                {
                    this.CodeCheckerDAO.UpdateAmountForOnlineTransaction(ENO, ReversalENO, BranchCode, ListIBLTLFDTO[0].Amount);
                }
            }
        ...
        public void MultipleTransactionReversalProcess(string ENO, string ReversalENO, string GroupNo, string BranchCode, DateTime Date, string ActiveBranch, int UpdatedUserID, TLMDTO00015 casndenodto, string Trancode, bool IsCbalChanges)
        {
            string splitStatus = string.Empty; //to check online withdraw reversal case
            #region validation
            if (string.IsNullOrEmpty(ENO))
            {
                throw new Exception("ME90018");//Invalid Parameter Value.
            ...
            if (ActiveBranch == BranchCode)
            {
                ...
                //Check CashDeno is Single or Multiple if null or empty == single
                TLMDTO00015 Orgncashdenodto = new TLMDTO00015();
                TLMDTO00009 DepoDenodto = new TLMDTO00009();              

                if (!string.IsNullOrEmpty(GroupNo))
                {
                    //Select Original Cash Deno for Payment or Receipt 
               
                    //TLMDTO00015 Orgncashdenodto1 = this.CodeCheckerDAO.SelectCashDenoByGroupNo(GroupNo, BranchCode);
                    Orgncashdenodto = this.CodeCheckerDAO.SelectCashDenoGroupTransactionByGroupNo(GroupNo, BranchCode);
                    DepoDenodto = this.CodeCheckerDAO.SelectDepoDenoReverseStatusByEntryNoAndGroupNo(GroupNo, ENO, BranchCode);
                    //Check Orgncashdenodto is null or empty
                    //if (Orgncashdenodto != null && !string.IsNullOrEmpty(Orgncashdenodto.TlfEntryNo)
                    if (DepoDenodto.Reverse_Status == false)
                    {
                        this.CodeCheckerDAO.UpdateDepoDenoByTLF_EnoGropuNoSourceBr(ENO, GroupNo, BranchCode, UpdatedUserID);

                        //Update Cash Deno by Reverse =true for Group eno // Modified by ZMS (coz it is also include in SP_InsertCashDeno_MultipleWithdrawAndDepositReversal)
                        //this.CodeCheckerDAO.UpdateCashDenoReverseStatusOfGroupByENO(GroupNo, ReversalENO, BranchCode, UpdatedUserID);

                        //Insert Cash Deno
                        #region Convert CashDeno DTO to CashDeno ORM
                        TLMORM00015 CashDenoORM = new TLMORM00015();
                        CashDenoORM.TlfEntryNo = GroupNo;
                        CashDenoORM.DenoEntryNo = ReversalENO;
                        string accoutType = DepoDenodto.AccountType.ToString();
                        CashDenoORM.AccountType = accoutType;
                        decimal amount = 0;
                        string denoRefundDetail = "", denoDetail = "", denoRate="";
                        if (casndenodto == null)
                        {
                            amount = 0;
                            denoRefundDetail = "";
                        ...
                        {
                            amount = casndenodto.Amount;
                            denoRefundDetail = casndenodto.DenoRefundDetail;
                            denoDetail = casndenodto.DenoDetail;
                            denoRate = casndenodto.DenoRate;
                        }
                        CashDenoORM.Amount = amount;
                        CashDenoORM.AdjustAmount = Orgncashdenodto.AdjustAmount;
                        CashDenoORM.DenoDetail = denoDetail;
                        CashDenoORM.DenoRate =denoRate;
                        CashDenoORM.DenoRefundDetail = denoRefundDetail;
                        CashDenoORM.DenoRefundRate = Orgncashdenodto.DenoRefundRate;
                        CashDenoORM.UserNo = Convert.ToString(Orgncashdenodto.CreatedUserId);
                        CashDenoORM.CounterNo = Orgncashdenodto.CounterNo;
                        CashDenoORM.Status = Orgncashdenodto.Status;
                        CashDenoORM.SourceBranchCode = Orgncashdenodto.SourceBranchCode;
                        CashDenoORM.Currency = Orgncashdenodto.Currency;
                        CashDenoORM.Reverse = false;
                        CashDenoORM.Rate = Orgncashdenodto.Rate;
                        CashDenoORM.Active = true;
                        CashDenoORM.CreatedUserId = Orgncashdenodto.CreatedUserId;
                        CashDenoORM.SettlementDate = Orgncashdenodto.SettlementDate;
                        #endregion Convert CashDeno DTO to CashDeno ORM
                        string result = cashDAO.InsertCashDeno_MultipleWithdrawAndDepositReversal(CashDenoORM);
        ...
        public void ReversalProcessForReceipt(string ENO, string ReversalENO, string GroupNo, string BranchCode,string chequeNo, DateTime Date, string ActiveBranch, int UpdatedUserID, TLMDTO00015 casndenodto, string Trancode, bool IsCbalChanges)
        {
            #region validation
            if (string.IsNullOrEmpty(ENO))
            {
            ...
            //Select TLF By ENO, ActiveBranch, Date
            ListTLFdto = this.CodeCheckerDAO.SelectTLFByENOBranchCodeDate(ENO, BranchCode, Date, Trancode);
            for (int i = 0; i < ListTLFdto.Count; i++)
            {
                ListTLFdto[i].NewChequeNo = chequeNo;
            }
            //Check List TLF DTO count is Less than Equal 0 or not 
            if (ListTLFdto.Count <= 0)
            {
            ...
            foreach (PFMDTO00054 tlfdto in ListTLFdto)
            {
                ...
                ReversalTlfORM.SourceCurrency = tlfdto.SourceCurrency;
                ReversalTlfORM.CurrencyCode = tlfdto.CurrencyCode;
                ReversalTlfORM.Cheque = tlfdto.NewChequeNo;
                ReversalTlfORM.PaymentOrderNo = tlfdto.PaymentOrderNo;
                ReversalTlfORM.DRegisterNo = tlfdto.DRegisterNo;
                ReversalTlfORM.ERegisterNo = tlfdto.ERegisterNo;
                ReversalTlfORM.LgNo = tlfdto.LgNo;
        ...
        [Transaction(TransactionPropagation.Required)]
        public IList<TLMDTO00018> SelectLoanGuarantee(string acctno)
        {
        ...
        public bool CheckChequeBookNo(string chequeBookNo, string branchCode)
        {
            PFMDTO00006 chequeDTO = this.codeCheckerDAO.GetChequeInfoByChequeBookNo(chequeBookNo, branchCode);
            if (chequeDTO != null)
            {
                this.ServiceResult.ErrorOccurred = true;
                this.ServiceResult.MessageCode = CXMessage.MV00227; // This Cheque Book No is already used.
                return true;
            }
            return false;
        }
        ...
        {
            return this.codeCheckerDAO.GetExpireAmount(accountNo);
        }

        //Added by AAM (31_July_2018)
        public bool IsExpiredLoansAccount_ForAllowCrTrans(string accountNo, DateTime todaydate)
        {
            int count = this.codeCheckerDAO.GetExpiredLoansFromLoansByAccountNo(accountNo, todaydate);
            if (count > 0)
            {
        ...
        //Added by HMW for "Seperating EOD" at 22-Apr-2019
        public TCMDTO00001 SelectStartBySourceBr(string sourcebr)
        {
            try
            {
                ...
                TCMDTO00001 startdto = this.codeCheckerDAO.SelectStartBySourceBr(sourcebr);
                if (startdto == null)
                {
                    this.ServiceResult.ErrorOccurred = true;
