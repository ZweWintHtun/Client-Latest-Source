...
namespace Ace.Cbs.Cx.Ser.Sve
{
    ...
    public class CXSVE00003 : BaseService, ICXSVE00003
    {
        ...
        [Transaction(TransactionPropagation.Required)]
        public bool UpdateDataAfterPrintingForCS(string accountNo, decimal printLineNo)
        {
            if (this.printingDAO.DeletePrnFileByAccountNo(accountNo))
                return this.printingDAO.UpdatePrintLineForCS(accountNo,printLineNo);
            else 
                return false;
        }

        [Transaction(TransactionPropagation.Required)]
        public bool UpdateDataAfterPrintingForFixed(string accountNo, decimal printLineNo)
        {
            if (this.printingDAO.DeleteFPrnFileByAccountNo(accountNo))
                return this.printingDAO.UpdatePrintLineForFixed(accountNo, printLineNo);
            else
                return false;
        }
