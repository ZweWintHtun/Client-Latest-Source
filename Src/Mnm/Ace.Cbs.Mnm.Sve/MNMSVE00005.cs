...
namespace Ace.Cbs.Mnm.Sve
{
    public class MNMSVE00005 : BaseService, IMNMSVE00005
    {
        ...
        public virtual string CheckDate(DateTime Dailydate, string sourcebr)
        {
            ...
            DateTime ActualYearEndDate = Convert.ToDateTime(selectdate); //Added by HMW (30-Jan-2019)
            int ActualYearEndYear=ActualYearEndDate.Year;
            ...
            // Modified By ZMS For Budget End Flexible 2018/09/17
            string Return = string.Empty;
            string budget = CcoaDAO.GetBudget_Month_Year_Quarter(7, Dailydate, sourcebr, Return); // For 2018/09/17 => 2018/04/01
            DateTime budgetDate = Convert.ToDateTime(budget);
            string BlF_Month = (budgetDate.Month - 1).ToString(); // 04 => 03
            int BlF_Day = System.DateTime.DaysInMonth(budgetDate.Year, budgetDate.Month - 1); // 03 => 31

            string nextdatestr; string lastdaystr;
            //string nextdatestr = "03";
            if (BlF_Month.Length == 1)
            {
                nextdatestr = "0" + BlF_Month.ToString(); // 3 =>03
            }
            else
            {
                nextdatestr = BlF_Month.ToString(); // 12 =>12
            }
            int nextmonth = Convert.ToInt32(nextdatestr);

            //string lastdaystr = "31";
            if (BlF_Day.ToString().Length == 1)
            {
                lastdaystr = "0" + BlF_Day.ToString(); // 3 =>03
            }
            else
            {
                lastdaystr = BlF_Day.ToString(); // 12 =>12
            }
            int lastday = Convert.ToInt32(lastdaystr);
            int datetxtday = Dailydate.Day;

            //string nextdaystr = "31";
            string nextdaystr = lastdaystr;
            int nextday = Convert.ToInt32(nextdaystr);

            //string message = "31/03/" + nextyear;            
            string message = nextdaystr + "/" + nextdatestr + "/" + nextyear;

            // Comment & Modify by HMW (31-Jan-2019) : the errow message "ME30005" was wrongly shown at Pristine at 30-Jan-2019.            
            string Str_EnterDate = Dailydate.ToString("MM/dd/yyyy");
            string Str_LastYearEndDate = ActualYearEndDate.ToString("MM/dd/yyyy");
            DateTime EnterDate = DateTime.Parse(Str_EnterDate);
            DateTime LastYearEndDate = DateTime.Parse(Str_LastYearEndDate);
            if (DateTime.Compare(EnterDate, LastYearEndDate) <= 0)
            {
                this.ServiceResult.ErrorOccurred = true;
                return this.ServiceResult.MessageCode = "ME30005";//Yearly posting is already finished.Please contact your administrator.
            }

            else if (lastmonth <= datetxtmonth && lastyear < datetxtyear && (datetxtyear - lastyear) != 1)
            {
                this.ServiceResult.ErrorOccurred = true;
                return this.ServiceResult.MessageCode = "MI30012," + message;// Please post balances for the year {0} first.
            }
        ...
        public virtual void Posting(DateTime startDate, DateTime endDate, string workstation, int createdUserId, string branchCode)
        {
            try
            {
                //Added by ZMS (17_Sep_2018)
                // Modified some parameters values pDate and branchCode by AAM(17_Sep_2018)
                string Return = string.Empty;
                //string budget1 = CcoaDAO.GetBudget_Month_Year_Quarter(1, DateTime.Now, "001", Return); // For 2018/09/17 => 18/18
                //string budget2 = CcoaDAO.GetBudget_Month_Year_Quarter(2, DateTime.Now, "001", Return); // For 2018/09/17 => 2018/2018
                string budget3 = CcoaDAO.GetBudget_Month_Year_Quarter(3, endDate, branchCode, Return); // For 2018/09/17 => 6
                //string budget4 = CcoaDAO.GetBudget_Month_Year_Quarter(4, DateTime.Now, "001", Return); // For 2018/09/17 => 2
                ...
                //Modified by AAM (17_Sep_2018)
                string sourceCur = "";
                if (postingList.Count == 0) sourceCur = "KYT";
                else sourceCur = postingList.Select(p => p.SOURCECUR).FirstOrDefault();
