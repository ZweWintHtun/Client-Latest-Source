...
namespace Ace.Cbs.Mnm.Sve
{
    public class MNMSVE00166 : BaseService, IMNMSVE00166
    {
        ...
        public TCMDTO00001 StartDTO { get; set; }//Added by HMW (29-May-2019) : To check reversal info for Seperating EOD
        //public IPFMDAO00056 Sys001DAO { get; set; }//Added by HMW (29-May-2019) : To check reversal info for Seperating EOD
        ...
        public IList<PFMDTO00054> Check_EntryNo_Valid(string groupNo, string eno, string sourceBr)
        {
            //ListTLFDTO = this.TlfDAO.GetTlfInfoByEntryNo(eno, sourceBr);
            ListTLFDTO = this.TlfDAO.GetTransactionInfoByEnoANDGroupNo(groupNo, eno, sourceBr);
            ...
            # region New_Checking_For_SeperatingEOD_ByHMW

            //string str = this.TlfDAO.Check_EntryNo_Multiple_DepWdwReverse(groupNo, eno, sourceBr);
            //if ((str == "-1"))
            //{
            //    this.ServiceResult.ErrorOccurred = true;
            //    this.ServiceResult.MessageCode = "MV30020"; //Invalid Group No.
            //    return null;
            //}
            //else if ((str == "-2"))
            //{
            //    this.ServiceResult.ErrorOccurred = true;
            //    this.ServiceResult.MessageCode = "MV00213"; //Invalid Entry No.
            //    return null;
            //}
            //else if ((str == "-3"))
            //{
            ...
            //{
            //    this.ServiceResult.ErrorOccurred = true;
            //    this.ServiceResult.MessageCode = "MV90202"; //Not Allow Back Date Transaction
            //    return null;
            //}
            //else if (str == "-8")
            //{
            //    this.ServiceResult.ErrorOccurred = true;
            //    this.ServiceResult.MessageCode = "MV90202"; //Not Allow Individuals Deposit/Withdraw Transaction!
            //    return null;
            //}
            //else
            # endregion
            ...
            //if (ListTLFDTO != null) // Entry No Exist
            if (ListTLFDTO.Count > 0)
            {
                ...
                {
                    this.ServiceResult.ErrorOccurred = true;
                    this.ServiceResult.MessageCode = "ME30018"; //Not Allowed Reversal for Invaid Date!
        ...
        public string Save_Reversal(string eno, string groupNo, int currentUserId, string sourceBr, TLMDTO00015 CashDenoDTO)
        {
            ...
            try
            {
                ...
                {                                                          
                    ...
                    //if (ListTLFDTO != null)
                    int i;
                    if (systemStartupDate != lastSettlementDate)
                    {
                        var isOrgnEnoNull = from x in ListTLFDTO
                                            where x.OrgnEno == null
                                            && x.DateTime.ToShortDateString() == ListTLFDTO[0].SettlementDate.Value.ToShortDateString() //DateTime.Now.ToShortDateString()
                                            select x;
                    ...
                    else
                    {
                        ...
                        if (ListIBLTLFDTO == null)//NOt IBL Transaction 
                        {
                            ...
                            //Comment & Modify by HMW (10-June-2019) : Seperating EOD Process
                            this.ReverSalDAO.MultipleTransactionReversalProcess(eno, GeneratedEno, groupNo, sourceBr, DateTime.Parse(ListTLFDTO[0].SettlementDate.ToString()), sourceBr, currentUserId, CashDenoDTO, string.Empty, true);  //8
        ...
        private string GetGeneratedEnoLocal(string MaxItem, IList<PFMDTO00054> ListTLFDTO)
        {
            ...
            DateTime lastSDate = CXCOM00011.Instance.GetScalarObject<DateTime>("Sys001.SelectSettlementDate", new object[] { CXCOM00007.Instance.GetValueByKeyName(CXCOM00009.NextSettlementDate), BranchCode, true });
            DateTime nextSDate = CXCOM00011.Instance.GetScalarObject<DateTime>("Sys001.SelectSettlementDate", new object[] { CXCOM00007.Instance.GetValueByKeyName(CXCOM00009.LastSettlementDate), BranchCode, true });            
            string systemStartupDate;
        ...
        {
            string str = this.depoDenoDAO.Check_GroupNo_ValidOrNot_MultipleDepReversal(groupNo, sourceBr);
            return str;
        }

        //Added by AAM(22-Jan-2018)
        //Commented by HMW (10-June-2019)
        //public string Check_EntryNo_ValidOrNot_MultipleDepReversal(string eno, string sourceBr,string groupNo)
        //{
        //    string str = this.depoDenoDAO.Check_EntryNo_ValidOrNot_MultipleDepReversal(eno, sourceBr, groupNo);
        //    return str;
        //}
        ...
        {
            string str = this.depoDenoDAO.Check_EntryNo_ValidOrNot_MultipleDepWdwReversal(groupNo, eno, sourceBr);
            return str;
        }

        //Added by HMW at 29-May-2019 (Reversal Validation : For Seperating EOD Process)
        public IList<PFMDTO00056> SelectAllAutoPayStatusList(string sourcebr)
        {  
            //IList<PFMDTO00056> AutoPayStatusList = Sys001DAO.SelectAllAutoPayStatusList(sourcebr);  
            IList<PFMDTO00056> AutoPayStatusList = IBLDAO.SelectAllAutoPayStatusList(sourcebr);

            //IList<PFMDTO00056> AutoPayStatusList = CXServiceWrapper.Instance.Invoke<IPFMDAO00056, IList<PFMDTO00056>>(x => x.SelectAllAutoPayStatusList(CurrentUserEntity.BranchCode));
        ...
        public bool CheckInfoBeforeReversal(DateTime txnSettlementDate,string sourceBr)
        {
            ...
            try
            {
                ...
                IList<PFMDTO00056> AutoPayRunList = CXServiceWrapper.Instance.Invoke<IMNMSVE00166, PFMDTO00056>(x => x.SelectAllAutoPayStatusList(sourceBr));

                if (systemDate.Date == DateTime.Now.Date)
                    settlementDate = nextSDate;
                else
                    settlementDate = lastSDate;
