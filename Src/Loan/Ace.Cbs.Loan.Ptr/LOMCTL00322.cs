...
{
    public class LOMCTL00322 : AbstractPresenter, ILOMCTL00322
    {
        #region Properties
        private ILOMVEW00322 view;
        public ILOMVEW00322 View
        {
            get { return this.view; }
            set { this.WireTo(value); }
        }

        private void WireTo(ILOMVEW00322 view)
        {
            if (this.view == null)
            {
                this.view = view;
        ...
        public void Print()
        {
            ...
            if (this.Validate_Form())
            {
                ...
                DTOList = CXClientWrapper.Instance.Invoke<ILOMSVE00316, IList<LOMDTO00316>>(x => x.SelectPLOverdrawnListingByCompanyName(DTO)).OrderBy(x=>x.PLNO).ToList();
                if (DTOList.Count > 0)
                {
                    string currency = this.view.Currency.ToString();
                    string sourcebranch = this.view.SourceBranch.ToString();
                    string header = "Personal Loans Overdrawn with Company Name A/C Listing For " + DateTime.Now.ToString("MMMM");
