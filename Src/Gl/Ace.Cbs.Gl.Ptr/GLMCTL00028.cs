...
namespace Ace.Cbs.Gl.Ptr
{
    public class GLMCTL00028 : AbstractPresenter, IGLMCTL00028
    {
        ...
        public string GetBudYear()
        {
            ...
            string activeBudget = CXClientWrapper.Instance.Invoke<IGLMSVE00028, string>(x => x.GetBudYear(6, DateTime.Now, this.View.CurrentBCode));
            return activeBudget;
        }
        public void Print()
        {
            if (this.Validate_Form())
            {
                ...
                else
                {
                    ...
                    IList<GLMDTO00028> lst_CA_OA = lst.Where(a => a.ACType == "CA").ToList();//Updated by ZMS(21-Jun-2018)

                    IList<GLMDTO00028> lst_CA_CCE = lst.Where(a => a.ACType == "CA" && a.SubACType == "CCE").ToList();

                    //IList<GLMDTO00028> lst_CA_ICR = lst.Where(a => a.ACType == "CA" && a.SubACType == "ICR").ToList(); //comment  by ZMS(21-Jun-2018)
                    ...
                    //DTO.ICRAmt = lst_CA_ICR[0].Amount;//commented by ZMS(21-Jun-2018)
                    ...
                    //Added by ZMS to get last day of require month (not holiday) 04.12.2018
                    string lastDayofRequireMonth = CXClientWrapper.Instance.Invoke<IGLMSVE00028, string>(x => x.GetLastDayofMonth(DTO.RequireYear , DTO.RequireMonth ));

                    string header = "Statement of Financial Position";
                    IList<GLMDTO00028> DTOList = new List<GLMDTO00028>();
                    lst.Add(DTO);
                    CXUIScreenTransit.Transit("frmGLMVEW00029.ReportViewer", true, new object[] { lst, header, lastDayofRequireMonth });
