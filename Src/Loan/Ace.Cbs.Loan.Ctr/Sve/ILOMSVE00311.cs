...
namespace Ace.Cbs.Loan.Ctr.Sve
{
    ...
    {
        IList<PFMDTO00072> IsValidForLoanAcctnoForHPRepaymentListing(string acctno); // Added by ZMS (03.12.18)for Pristine requirements
        IList<PFMDTO00072> IsValidForLoanAcctnoForPLRepaymentListing(string acctno); // Added by ZMS (16.11.18)for Pristine requirements

        IList<PFMDTO00072> IsValidForLoanAcctno(string acctno);
        IList<PFMDTO00028> IsValidForLoanAcctnoForPLContractPrinting(string acctno, string sourceBranchCode);
        ...
        string PLMonthlyAutoPaymentProc(string sourceBr, int createdUserId, string userName, IList<DataVersionChangedValueDTO> dvcvList); //Modified by HMW at (06-08-2019) : To prevent "Voucher No Loss Issue" in every already run (or) no need to run case.
        ...
        IList<LOMDTO00338> GetCustInfoByAcctNoForBL_LB(string acctNo, string sourceBr);//Added by YMP (19-Feb-2019)
        ...
        IList<LOMDTO00243> Get_PLInfoRegisterEdit_ByAcctNo(string acctNo, string sourceBr);//Added by AAM (22-May-2017)
