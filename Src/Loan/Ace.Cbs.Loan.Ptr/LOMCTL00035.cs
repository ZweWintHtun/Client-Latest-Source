...
{
    public class LOMCTL00035 : AbstractPresenter, ILOMCTL00035
    {
        #region Properties
        private ILOMVEW00035 view;
        public ILOMVEW00035 View
        {
            get { return this.view; }
            set { this.WireTo(value); }
        }

        private void WireTo(ILOMVEW00035 view)
        {
            if (this.view == null)
            {
                this.view = view;
                ...
                if (!CXCOM00006.Instance.IsExceedTodayDate(this.view.StartDate))
                {
                    if (!CXCOM00006.Instance.IsExceedTodayDate(this.view.EndDate))
                    {
                        ...
                        else
                        {
                            ...
                            if (loansType == "All")
                            {
                                DTOList = CXClientWrapper.Instance.Invoke<ILOMSVE00035, IList<LOMDTO00035>>(x => x.SelectAllODData(DTO));
                            }
                            else
                            {
                                DTOList = CXClientWrapper.Instance.Invoke<ILOMSVE00035, IList<LOMDTO00035>>(x => x.SelectODDataByLoansType(DTO));
                            }
                            if (DTOList.Count > 0)
                            {
                                ...
                                string header = "Overdraft Registration Listing";                                
