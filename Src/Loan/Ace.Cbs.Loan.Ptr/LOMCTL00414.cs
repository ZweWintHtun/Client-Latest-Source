...
{
    public class LOMCTL00414 : AbstractPresenter, ILOMCTL00414
    {
        #region Properties
        private ILOMVEW00414 view;
        public ILOMVEW00414 View
        {
            get { return this.view; }
            set { this.WireTo(value); }
        }

        private void WireTo(ILOMVEW00414 view)
        {
            if (this.view == null)
            {
                this.view = view;
            ...
            if (!CXCOM00006.Instance.IsExceedTodayDate(this.view.StartDate))
            {
                if (!CXCOM00006.Instance.IsExceedTodayDate(this.view.EndDate))
                {
                    ...
                    else
                    {
                        ...
                        DTOList = CXClientWrapper.Instance.Invoke<ILOMSVE00405, IList<LOMDTO00405>>(x => x.SelectBLRepaymentInfolistingByRepayDate(DTO));
                        if (DTOList.Count > 0)
                        {
                            ...
                            string header = "Business Loans Repayment Listing";
                            CXUIScreenTransit.Transit("frmLOMVEW00415.ReportViewer", true, new object[] { DTOList, startDate, endDate, currency, sourcebranch, header, BLType, this.view.rptname });
                        }
                        else
                        {
